...
const testSubmitNotes = async () => {
  try {
    ...
    const response = await axios.post('http://localhost:1111/onboarding/submit-notes', testData, {
      headers: {
...
const testSubmitNotes = async (formType) => {
  try {
    ...
    console.error(`❌ ${formType}: Error -`, error.response?.data?.error || error.message);
  }
};
