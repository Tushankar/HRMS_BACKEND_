...
router.post("/save-misconduct-statement", async (req, res) => {
  try {
    ...
    const message = status === "draft" ? "Misconduct statement saved as draft" : "Misconduct statement completed";
...
router.post("/save-code-of-ethics", async (req, res) => {
  try {
    ...
    const mappedData = {
      ...
      signatureDate: formData.date ? new Date(formData.date) : (formData.signatureDate ? new Date(formData.signatureDate) : null),
      status
    };
    ...
    const message = status === "draft" ? "Code of ethics saved as draft" : "Code of ethics completed";
...
router.post("/save-service-delivery-policy", async (req, res) => {
  try {
    ...
    const mappedData = {
      ...
      // Supervisor signature fields (frontend uses agencySignature, agencyDate)
      supervisorSignature: formData.agencySignature || '',
      supervisorSignatureDate: formData.agencyDate ? new Date(formData.agencyDate) : null,
      status
    };
    ...
    const message = status === "draft" ? "Service delivery policy saved as draft" : "Service delivery policy completed";
...
router.get("/get-service-delivery-policy/:applicationId", async (req, res) => {
  try {
    ...
    const serviceDeliveryPolicy = await ServiceDeliveryPolicy.findOne({ applicationId });
...
router.post("/save-non-compete-agreement", async (req, res) => {
  try {
    ...
    const mappedData = {
      ...
      effectiveDate: {
        day: formData.agreementDate ? new Date(formData.agreementDate).getDate() : null,
        month: formData.agreementMonth || (formData.agreementDate ? new Date(formData.agreementDate).toLocaleDateString('en-US', { month: 'long' }) : ''),
        year: formData.agreementYear ? parseInt(formData.agreementYear) : (formData.agreementDate ? new Date(formData.agreementDate).getFullYear() : null)
      },
      ...
      // Signature mapping
      employeeSignature: formData.employeeSignature || '',
      employeeSignatureDate: formData.agreementDate ? new Date(formData.agreementDate) : null,
      // Company representative mapping
      companyRepresentative: {
        name: formData.companyRepName || '',
        signature: formData.companyRepSignature || ''
      },
      status
    };
    ...
    const message = status === "draft" ? "Non-compete agreement saved as draft" : "Non-compete agreement completed";
...
router.get("/get-non-compete-agreement/:applicationId", async (req, res) => {
  try {
    ...
    const nonCompeteAgreement = await NonCompeteAgreement.findOne({ applicationId });
    
    if (!nonCompeteAgreement) {
      return res.status(404).json({ message: "Non-compete agreement not found" });
    }
