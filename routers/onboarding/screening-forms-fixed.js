...
router.post("/save-background-check", async (req, res) => {
  try {
    ...
    if (hrFeedback && !formData) {
      ...
      if (!form) {
        return res.status(404).json({ success: false, message: "Background check form not found" });
      }
    ...
    if (!applicationId && !employeeId) {
      return res.status(400).json({ success: false, message: "Application ID or Employee ID is required" });
    }
    ...
    if (backgroundCheckForm) {
      ...
      if (formData.applicantSignature) backgroundCheckForm.applicantSignature = formData.applicantSignature;
      if (formData.applicantSignatureDate) backgroundCheckForm.applicantSignatureDate = formData.applicantSignatureDate;
