...
async function quickTest() {
  ...
  try {
    ...
    const empApp = await axios.post(`${BASE_URL}/save-employment-application`, {
      ...
      formData: {
        personalInfo: {
          ...
          address: { street: "123 Test St", city: "Test City", state: "CA", zipCode: "12345" },
    ...
    console.log(`✅ Employment Application: ${empApp.status} - ${empApp.data.completionPercentage}%`);
    ...
    console.log(`✅ I-9 Form: ${i9Form.status} - ${i9Form.data.completionPercentage}%`);
    ...
    console.log(`✅ W-4 Form: ${w4Form.status} - ${w4Form.data.completionPercentage}%`);

    // Step 5: Test Emergency Contact
    console.log('\n5️⃣ Testing Emergency Contact...');
    const emergencyContact = await axios.post(`${BASE_URL}/save-emergency-contact`, {
      applicationId: APPLICATION_ID,
      employeeId: EMPLOYEE_ID,
      formData: {
        employeeInfo: {
    ...
    console.log(`✅ Emergency Contact: ${emergencyContact.status} - ${emergencyContact.data.completionPercentage}%`);

    // Step 6: Submit Application
    console.log('\n6️⃣ Submitting Application...');
    const submitResponse = await axios.put(`${BASE_URL}/submit-application/${APPLICATION_ID}`);
    console.log(`✅ Application Submitted: ${submitResponse.data.application.applicationStatus}`);

    // Step 7: Test HR Functions
    console.log('\n7️⃣ Testing HR Functions...');
    const allApps = await axios.get(`${BASE_URL}/get-all-applications`);
    console.log(`✅ Retrieved ${allApps.data.applications.length} applications`);
