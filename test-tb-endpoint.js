...
async function testTBSymptomScreen() {
  try {
    ...
    const response = await axios.post(`${BASE_URL}/save-tb-symptom-screen`, testData);
    ...
    if (response.data.tbSymptomScreen) {
      ...
      if (response.data.tbSymptomScreen.basicInfo) {
        console.log('✅ Basic Info mapped:', response.data.tbSymptomScreen.basicInfo.fullName);
      }
      if (response.data.tbSymptomScreen.symptoms) {
        console.log('✅ Symptoms mapped:', Object.keys(response.data.tbSymptomScreen.symptoms));
      }
      if (response.data.tbSymptomScreen.actionTaken) {
        console.log('✅ Action Taken mapped:', Object.keys(response.data.tbSymptomScreen.actionTaken));
      }
    }
