  ...
  try {
    ...
    for (let i = 0; i < forms.length; i++) {
      ...
      try {
        ...
        // Retrieve form
        const getResponse = await axios.get(`${BASE_URL}/get-${formName}/${APPLICATION_ID}`);
        console.log(`  📥 Get: ✅ Status ${getResponse.status}`);

        // Complete form
        const completeResponse = await axios.post(`${BASE_URL}/save-${formName}`, {
          applicationId: APPLICATION_ID,
          employeeId: EMPLOYEE_ID, 
          formData: testData[formName],
          status: 'completed'
        ...
        console.log(`  📊 Progress: ${completeResponse.data.completionPercentage}%`);
    ...
    try {
      const submitResponse = await axios.put(`${BASE_URL}/submit-application/${APPLICATION_ID}`);
      console.log('✅ Application submitted successfully');
      console.log(`   Status: ${submitResponse.data.application.applicationStatus}`);
      passedTests++;
    } catch (error) {
    ...
    try {
      ...
      const allAppsResponse = await axios.get(`${BASE_URL}/get-all-applications`);
      console.log(`✅ Retrieved ${allAppsResponse.data.applications.length} applications`);

      // Update application status
      const statusResponse = await axios.put(`${BASE_URL}/update-status/${APPLICATION_ID}`, {
        status: 'under_review',
        reviewComments: 'Test review comment',
        reviewedBy: EMPLOYEE_ID
      });
      console.log('✅ Application status updated');
      console.log(`   New Status: ${statusResponse.data.application.applicationStatus}`);

      passedTests += 2;
    } catch (error) {
      console.log('❌ HR functions failed:', error.response?.data?.message || error.message);
      failedTests += 2;
    }
