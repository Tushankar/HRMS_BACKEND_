...
const testBackgroundCheckForm = async () => {
  try {
    ...
    const saveResponse = await axios.post(`${baseURL}/onboarding/save-background-check`, {
    ...
    const getResponse = await axios.get(`${baseURL}/onboarding/get-background-check-by-id/${savedFormId}`);
    
    console.log('✓ Get Response:', getResponse.data.message);
    console.log('✓ Form data retrieved successfully');
    console.log(`✓ Retrieved form for: ${getResponse.data.backgroundCheck.formData.firstName} ${getResponse.data.backgroundCheck.formData.lastName}\n`);
    
    // Test updating the form
    console.log('3. Testing form update...');
    const updatedFormData = { ...testFormData, lastName: "Johnson" };
    const updateResponse = await axios.post(`${baseURL}/onboarding/save-background-check`, {
      applicationId: testApplicationId,
      employeeId: testEmployeeId,
    ...
    console.log(`✓ Form status updated to: ${updateResponse.data.backgroundCheck.status}\n`);
